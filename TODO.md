...
- [ ] **Pre-C07: real String/Array/Slice runtime**
	...
	- **PROGRESS**: parser AST 已保留 `StringPart_Chunk` / `StringPart_Expr` / `StringPart_End`，`lower_ast` 会把纯 chunk literal 合并为 `L0/L1OpStringBytes(Str)`，并把插值字符串保留为 `L0/L1OpStringInterp` 而不是静默降成空数组。WAT emitter 对纯 literal 输出真实 `array.new_fixed $array_u8 N` byte payload；raw string smoke 现在能看到 `raw ${text} stays raw` 的 21 字节 payload。typed `String` 参数会降低成 `(ref $array_u8)`，`s[i]` 会生成 `array.get_u $array_u8` + `i64.extend_i32_u`，`s[a..b]` 会生成 `$slice_u8` view；`.char_at` / `.codepoint_at` 会调用 UTF-8 codepoint helper。插值字符串现在把 chunk 和 String 表达式通过 `__chiba_string_concat2` 复制到新的 `$array_u8`；payload-less legacy `L1OpString` 不再 emit 空数组。
	- **NEXT**: 显式 bounds diagnostic/trap policy；把 WASI read/write bridge 和 lexer/parser input 改成同一 Array/Slice contract；给 String/Slice 返回值和跨函数调用补全 typed result ABI。
